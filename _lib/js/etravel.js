 jQuery(document).ready(function() {
     ...
     jQuery("#formahotel .EtDateToGN, #formapackage .EtDateToGN,#formacar .EtDateToGN").datepicker("option", {
         beforeShow: function() {
     ...
     if ($("#formaflight").length === 1) {
         ...
         //Evitar que se realize el submit en este campo 
         jQuery("#EtCityOrigFL,#EtDestinyFL").keypress(function(e) {
             if (e.which == 13) e.preventDefault()
         });
         ...
         jQuery("#EtCityOrigFL").autocomplete({
             ...
             select: function(event, ui) {
                 ...
                 inputText = ui.item.Label;
                 jQuery("#EtCityOrigFL").blur(); //Para que se limpie el input cuando se le de click
                 return false;
             }
         }).data("ui-autocomplete")._renderItem = function(ul, item) {
             return jQuery("<li></li>")
         ...
         jQuery("#EtDestinyFL").autocomplete({
             ...
             select: function(event, ui) {
                 ...
                 inputText = ui.item.Label;
                 jQuery("#EtDestinyFL").blur(); //Para que se limpie el input cuando se le de click
                 return false;
             }
         }).data("ui-autocomplete")._renderItem = function(ul, item) {
             return jQuery("<li></li>")
     ...
     if ($("#formacar").length === 1) {
         ...
         jQuery("#cityco,#cityib").keypress(function(e) {
             if (e.which == 13) e.preventDefault()
         });
     ...
     if ($("#formatour").length === 1) {
         jQuery('#formatour').submit(function(e) {
             return (ValidateDate('formatour'));
         });
         jQuery("#formatour .EtDateFromGN, #formatour .EtDateToGN").datepicker("option", {
             beforeShowDay: null,
             beforeShow: null,
             onClose: null
         });
     ...
     if ($("#formatransfer").length === 1) {
         jQuery('#formatransfer').submit(function(e) {
             return (ValidateHotel('formatransfer', 'EtHotel', FalseHotel, MsjHotel));
         });
         changeFocus("#EtHotel", FalseHotel);
         //Calendarios
         jQuery("#formatransfer .EtDateFromGN").datepicker("option", "minDate", 1); //minDate se indica el dia minimo para reservar, en este caso un dia mas a la fecha actual
         ...
         jQuery("#EtHotel").keypress(function(e) {
             if (e.which == 13) e.preventDefault()
         });
