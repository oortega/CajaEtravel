...
//ESTA SECCIÓN IDENTIFICA A QUE CALENDARIO SE LE DA CLICK
	if (dtClass.indexOf('EtDateFromGN')>=0){
		var dateTextFrom=jQuery(this).val();
		var dateTextTo=dateToInput.val();
		var date = new Date(dateSel); 
		jQuery.datepick.add(date,'+1', 'd');
		if (dateTextTo=="") {
			dateToInput.val(jQuery.datepick.formatDate(FormatO, date));
			dateTextTo = dateToInput.val();
		}
		var dateFrom = $.datepick.parseDate(FormatO, dateTextFrom );
		var dateTo = $.datepick.parseDate(FormatO, dateTextTo );
		if (dateFrom>=dateTo) {dateToInput.val(jQuery.datepick.formatDate(FormatO, date));}
	}
	else
	{
		var dateTextFrom=dateFromInput.val();
		var dateTextTo=jQuery(this).val();
		var date = new Date(dateSel);
		jQuery.datepick.add(date,'-1', 'd');
		if (dateTextFrom=="") {
			dateFromInput.val(jQuery.datepick.formatDate(FormatO, date));
			dateTextFrom = dateFromInput.val();
		}
		var dateFrom = $.datepick.parseDate(FormatO, dateTextFrom );
		var dateTo = $.datepick.parseDate(FormatO, dateTextTo );
		if (dateTo<=dateFrom) {dateFromInput.val(jQuery.datepick.formatDate(FormatO, date));}
	}
}
//Fechas default
function nextDay(day){  // Función para buscar el siguiente día de la semana
    var d = new Date();
...
function DefaultDate()
{
	...
	defaultDate = nextDay(5);       // Obtiene la fecha del siguiente viernes
	defaultDate = jQuery.datepick.add(defaultDate,'+7', 'd');		// Le suma 7 días
	//defaultDate = new Date();		// La fecha actual
	//defaultDate = jQuery.datepick.add(defaultDate,'+7', 'd');		// Le suma 7 días
	jQuery(".EtDateFromGN").val(jQuery.datepick.formatDate(FormatO, defaultDate));		//	Asigna la hora default FROM
	defaultDate = jQuery.datepick.add(defaultDate,'+2', 'd');		// Le suma 2 día
	jQuery(".EtDateToGN").val(jQuery.datepick.formatDate(FormatO, defaultDate));		// Asigna la hora default TO
}
//Ajustes del calendario
function NewControlsCalendar()
{	
	...
	jQuery("#datepick-nav").remove();
	jQuery(".datepick-nav").before('<div id="datepick-nav"><a href="javascript:void(0)" id="datepick-cmd-prev"></a><div class="NewControlsCalendar"><span class="dtpkmes">'+dtpkmes+' '+dtpkanio+'</span><span class="month2">'+month2+'</span></div><a href="javascript:void(0)" id="datepick-cmd-next"></a></div>');
	if (jQuery(".datepick-cmd-prev").hasClass("datepick-disabled")) {jQuery("#datepick-cmd-prev").addClass("datepick-disabled")}
	if (jQuery(".datepick-cmd-next").hasClass("datepick-disabled")) {jQuery("#datepick-cmd-next").addClass("datepick-disabled")}	
}
jQuery(function() {
	jQuery("body").delegate("#datepick-cmd-prev", "click", function(){
		jQuery('.datepick-cmd.datepick-cmd-prev').click();
		NewControlsCalendar();
	});
	jQuery("body").delegate("#datepick-cmd-next", "click", function(){
		jQuery('.datepick-cmd.datepick-cmd-next').click();
		NewControlsCalendar();
	});
});
//Modificar el foco
...
// Función para calcular si un día está seleccionado
function RangoDias(date) {
	var clase="";
	var titulo="";
 	var forma=jQuery(this).parents("form").attr('id');
 	var inicio=$.datepick.parseDate(FormatO, $('#'+forma+' .EtDateFromGN').val());
 	var fin=$.datepick.parseDate(FormatO, $('#'+forma+' .EtDateToGN').val());
 	date=parseInt(date.getTime());
 	inicio=parseInt(inicio.getTime());
 	fin=parseInt(fin.getTime());
    ...
    return {selectable: true, dateClass: clase, title: titulo}; 
}
//Validar Fechas
...
// Config generales
	...
	//Calendarios
	jQuery(".EtDateFromGN, .EtDateToGN").datepick({
		dateFormat: FormatO,
		firstDay: 1,
		showOnFocus: true,
	...
	jQuery(".etWContainer").find(".etWSelect select").change();
		...
		jQuery("#EtDestinyHtl").catcomplete({
			...
			select: function(event, ui) {
				//alert(ui.item.Label);
					...
					if(ui.item.Type=="H"){ jQuery("#EtHt").val(ui.item.TypeID); jQuery("#formahotel").attr('action',SubDominioetw+'/Hoteles/Tarifas'); }
					else { jQuery("#EtHt").val(""); jQuery("#formahotel").attr('action',SubDominioetw+'/Hoteles/Lista'); }
					jQuery("#EtCt").val(ui.item.Country);
					return false;
			} 
		});
		//Calendarios
		jQuery("#formahotel .EtDateFromGN").datepick("option", {minDate:0,maxDate:"+1y"});
		jQuery("#formahotel .EtDateToGN").datepick("option", {minDate:+1,maxDate:"+1y +1d"});
	}
...
// Config de form paquetes
	if ($("#formapackage").length === 1) {	
		...
		//Calendarios
		jQuery("#formapackage .EtDateFromGN").datepick("option", {minDate:0,maxDate:"+1y"});
		jQuery("#formapackage .EtDateToGN").datepick("option", {minDate:+1,maxDate:"+1y +1d"});
	}
...
// Config de form vuelos
	if ($("#formaflight").length === 1) {	
		...
		//Calendarios
		jQuery("#formaflight .EtDateFromGN").datepick("option", {minDate:0,maxDate:"+1y"});
		jQuery("#formaflight .EtDateToGN").datepick("option", {minDate:+1,maxDate:"+1y +1d"});
	}	
...
// Config de form autos
	if ($("#formacar").length === 1) {	
		...
		//Calendarios
		jQuery("#formacar .EtDateFromGN").datepick("option", {minDate:1,maxDate:"+1y"});
		jQuery("#formacar .EtDateToGN").datepick("option", {minDate:+2,maxDate:"+1y +1d"});
	}
...
// Config de form tours
	if ($("#formatour").length === 1) {	
		...
		//Calendarios
		jQuery("#formatour .EtDateFromGN").datepick("option", {minDate:0,maxDate:"+1y",onDate:null});
	}
	...
	if ($("#formatransfer").length === 1) {	
		...
		jQuery("#EtTypeId").change(function(){
			if(jQuery(this).val()=="R"){ 
				...
				jQuery("#formatransfer .EtDateFromGN,#formatransfer .EtDateToGN ").datepick("option", {onDate:RangoDias});
			}
			if(jQuery(this).val()=="L"){ 
				jQuery("#formatransfer .EtDateToGN").parent().hide();
				jQuery("#formatransfer .EtDateFromGN").parent().show();
				jQuery("#formatransfer .EtDateFromGN,#formatransfer .EtDateToGN ").datepick("option", {onDate:null});
			}
		...
		//Calendarios
		jQuery("#formatransfer .EtDateFromGN").datepick("option", {minDate:2,maxDate:"+1y"});
		jQuery("#formatransfer .EtDateToGN").datepick("option", {minDate:3,maxDate:"+1y +1d"});
	}
	ResetAll();
	DefaultDate();
});